...
const cameraOnIcon = `<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg>`;
const cameraOffIcon = `<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M1 1l22 22" />\<polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg>`;
const audioOnIcon = `<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"></path><path d="M19 10v2a7 7 0 0 1-14 0v-2"></path><line x1="12" y1="19" x2="12" y2="23"></line><line x1="8" y1="23" x2="16" y2="23"></line></svg>`;
const audioOffIcon = `<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 9v3a3 3 0 0 0 6 0v-1" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /><line x1="1" y1="1" x2="23" y2="23" /> \</svg>`;
...
document.getElementById('create-meeting').onclick = () => {
  roomId = 'meet-' + Math.random().toString(36).substring(2, 9); // random id
  console.log("meeting id Created", roomId)
  // Set meeting ID in input field for easy copying/sharing
  const meetingInput = document.getElementById('meeting-id-input');
  meetingInput.value = roomId;
  meetingInput.select(); // Optional: select for quick copy
  isHost = true;
  // startMeeting();
};

document.getElementById('join-meeting').onclick = () => {
  roomId = document.getElementById('meeting-id-input').value.trim();
  console.log("joined with id:", roomId)
  if (roomId) {
    isHost = false;
    document.getElementById('meeting-entry').style.display = 'none';
  } else {
...
function startMeeting() {
  document.getElementById('create-meeting').disabled = true;
  document.getElementById('join-meeting').disabled = true;
  document.getElementById('meeting-id-input').disabled = true;
  // Now run original meeting logic, e.g. your (async () => { ... }) IIFE
  initMeeting();
}
